...
export async function weightCommand(ctx: Context): Promise<void> {
  ...
  if (isNaN(weight)) {
    await ctx.reply(
      `❌ *"${weightStr}" bukan angka yang valid!*\n\n` +
  ...
  if (weight < 20 || weight > 400) {
    await ctx.reply(
      `❌ *Berat tidak masuk akal: ${weight} kg*\n\n` +
  ...
  try {
    await logWeight(telegramId, weight);

    await ctx.reply(
      `✅ *${weight} kg berhasil dicatat!*\n\n` +
