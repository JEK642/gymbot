...
import { mainMenuKeyboard, workoutTypeKeyboard, backToMenuKeyboard, statsKeyboard } from '../keyboards/mainMenu';
...
export function registerCallbackHandlers(bot: Telegraf): void {
  ...
  bot.action('menu_workout', async (ctx) => {
    ...
    await editOrReply(
      ...
      { parse_mode: 'Markdown', ...workoutTypeKeyboard }
    );
  });
  ...
  bot.action('menu_stats', async (ctx) => {
    ...
    try {
      ...
      await editOrReply(
        ...
        {
          parse_mode: 'Markdown',
          ...statsKeyboard,   // ← DIUBAH: dari backToMenuKeyboard ke statsKeyboard
        }
      );
    } catch (error) {
      console.error('menu_stats callback error:', error);
      await editOrReply(ctx, '⚠️ Gagal memuat stats. Coba lagi.', backToMenuKeyboard);
    }
  });
