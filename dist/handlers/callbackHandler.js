...
const workoutService_2 = require("../services/workoutService");
const weightService_2 = require("../services/weightService");
// ============================================
...
function registerCallbackHandlers(bot) {
    ...
    bot.action('menu_main', async (ctx) => {
        ...
        await ctx.editMessageText(`🏠 *Menu Utama — ${firstName}*\n\n` +
    ...
    bot.action('menu_weight', async (ctx) => {
        await ctx.answerCbQuery();
        await ctx.editMessageText(`⚖️ *Log Berat Badan*\n\n` +
    ...
    bot.action('menu_workout', async (ctx) => {
        await ctx.answerCbQuery();
        await ctx.editMessageText(`🏋️ *Pilih Tipe Workout Hari Ini*\n\n` +
    ...
    bot.action('menu_stats', async (ctx) => {
        ...
        try {
            const [latestWeightLog, weeklyWorkouts, totalWorkouts, weightProgress] = await Promise.all([
                (0, weightService_1.getLatestWeight)(telegramId),
                (0, workoutService_2.getWorkoutsThisWeek)(telegramId),
                (0, workoutService_2.getTotalWorkouts)(telegramId),
                (0, weightService_2.getWeightProgress)(telegramId),
            ]);
            ...
            await ctx.editMessageText('⚠️ Gagal memuat stats. Coba lagi.', mainMenu_1.backToMenuKeyboard);
        }
    });
    // Klik tombol "Bantuan"
    bot.action('menu_help', async (ctx) => {
        await ctx.answerCbQuery();
        await ctx.editMessageText(`❓ *Cara Pakai GymBot*\n\n` +
            `*Perintah dasar:*\n` +
    ...
    workoutTypes.forEach((type) => {
        bot.action(`workout_${type}`, async (ctx) => {
            ...
            try {
                ...
                await (0, workoutService_1.logWorkout)(telegramId, type, null, null, null);
                const hype = workoutHype[type] ?? `🏋️ Sesi ${type.toUpperCase()} berhasil dicatat!`;
                await ctx.editMessageText(`✅ *Workout Tercatat!*\n\n` +
                    `${hype}\n\n` +
